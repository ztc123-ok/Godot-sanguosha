...
func _test_iron_chain_and_recast() -> void:
	...
	game.request_card_use(0)
	_expect(game.flow_state == GameManager.FlowState.SELECTING_TARGET, "【铁索连环】进入点击选目标状态")
	game.request_target(0)
	game.request_target(1)
	_expect(game.iron_chain_selected_targets.size() == 2, "【铁索连环】可点击选择两名角色")
	game.request_target(1)
	_expect(game.iron_chain_selected_targets == [p1], "重复点击【铁索连环】目标可取消选择")
	game.request_target(1)
